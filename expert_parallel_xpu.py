...
class XPUExpertParallel(ParallelStyle):
    ...
    def _all_gather_dispatch(
        ...
        for src_rank in range(ep_size):
            ...
            src_offsets = torch.cat(
                [torch.tensor([0], device=tensor.device), src_cumsum]
            )
        ...
        # Save state for combine - detach to prevent autograd graph retention
        self._dispatch_state = {
            "global_ntpe": global_ntpe.detach(),  # Detach to prevent memory leak
            "ep_size": ep_size,
            "ep_rank": ep_rank,
    ...
    def _all_gather_combine(
        ...
        gathered_outputs_list = [
            torch.zeros_like(padded_output) for _ in range(ep_size)
        ]
        ...
        for expert_id in range(num_experts):
            ...
            target_experts_range = slice(
                target_rank * num_local_experts, (target_rank + 1) * num_local_experts
            )
            tokens_from_prev_ranks = (
                global_ntpe[:ep_rank, target_experts_range].sum().item()
            )
            ...
            tokens_from_me_prev_experts = (
                global_ntpe[ep_rank, target_rank * num_local_experts : expert_id]
                .sum()
                .item()
            )
        ...
        # Simply set to None - the tensors will be garbage collected
    ...
    def _token_dispatch(
        ...
        dispatched = self._all_gather_dispatch(
            routed_input, num_tokens_per_expert, device_mesh
        )
...
def get_expert_parallel_class(force_native: Optional[bool] = None):
    """
    Get the appropriate ExpertParallel class for the current device.

    Args:
        force_native: If True, use native ExpertParallel (all_to_all-based).
                     If False, use XPUExpertParallel (all_gather-based) on XPU.
                     If None (default), use native EP (recommended for XCCL 25.190.0+).

    Returns:
        ExpertParallel class (either XPUExpertParallel or upstream ExpertParallel)

    Environment Variables:
        TORCHTITAN_XPU_FORCE_WORKAROUND_EP: Set to "1" to force XPUExpertParallel
                                            (all_gather-based) on XPU. This is
                                            deprecated and may hang after a few steps.

    Note:
        As of XCCL 25.190.0, native all_to_all works correctly. The XPUExpertParallel
        workaround is deprecated and may have stability issues (hangs at step 4+).
        Use native ExpertParallel (default) for best results.
    """
    import os

    # Check for deprecated workaround flag
    force_workaround = os.environ.get("TORCHTITAN_XPU_FORCE_WORKAROUND_EP", "0") == "1"
    # Also check old env var for backwards compatibility
    force_native_env = os.environ.get("TORCHTITAN_XPU_FORCE_NATIVE_EP", "0") == "1"

    if force_native is None:
        # Default: use native EP (all_to_all) since XCCL 25.190.0 supports it
        # Only use workaround if explicitly requested
        force_native = not force_workaround

    if is_xpu_available() and not force_native:
        print(
            "[EP] WARNING: Using deprecated XPUExpertParallel (all_gather-based). "
            "This may hang after a few training steps. "
            "Remove TORCHTITAN_XPU_FORCE_WORKAROUND_EP to use native ExpertParallel.",
            flush=True,
        )
        ...
        if is_xpu_available():
            print(
                "[EP] Using native ExpertParallel (all_to_all) on XPU "
                "(recommended for XCCL 25.190.0+)",
                flush=True,
            )
