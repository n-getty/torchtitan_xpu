#!/bin/bash
# EP=12 Benchmark Launch Script (MoE Fallback - No Triton)
# Uses XPUExpertParallel (Optimized AllGather) + PyTorch MoE (No Triton)
...
# Add repo root and torchtitan root to PYTHONPATH so we can import 'torchtitan' and 'torchtitan_xpu'
...
# Run on 12 processes (one per tile matches 12 ranks on 1 node)
echo "Launching EP=12 Benchmark (Fallback MoE) on 12 XPU ranks..."
$MPIEXEC -n 12 -ppn 12 $PYTHON_EXE -u $BASE_DIR/mpi_train_wrapper.py \
    --job.config_file $BASE_DIR/torchtitan/models/llama4/train_configs/llama4_ep12_fallback_xpu.toml \
    --training.steps=20
