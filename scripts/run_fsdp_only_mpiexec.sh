#!/bin/bash
# FSDP-only Benchmark with mpiexec launch (EP disabled)
...
# Run with mpiexec: 2 processes, 2 per node (single node), FSDP only (no EP)
$MPIEXEC -n 2 -ppn 2 $PYTHON_EXE -u $BASE_DIR/mpi_train_wrapper.py \
    --job.config_file $BASE_DIR/torchtitan/models/llama4/train_configs/llama4_fsdp_only_xpu.toml \
    --training.steps=5
