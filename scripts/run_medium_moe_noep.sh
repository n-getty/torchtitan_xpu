#!/bin/bash
# Medium Model MoE (No EP) Benchmark Launch Script
# Uses mediummodel_12_rank with EP=1 (experts replicated on all ranks)
...
echo "Launching Medium Model MoE (No EP) Benchmark on 12 XPU ranks..."
$MPIEXEC -n 12 -ppn 12 $PYTHON_EXE -u $BASE_DIR/mpi_train_wrapper.py \
    --job.config_file $BASE_DIR/torchtitan/models/llama4/train_configs/llama4_medium_moe_noep_xpu.toml \
    --training.steps=20
