...
try:
    from torchtitan.experiments.kernels.moe.triton_fused_moe_xpu import fused_moe
except ImportError:
    fused_moe = None
...
class TokenChoiceTopKRouter(nn.Module):
    ...
    def forward(
        ...
        return top_scores, token_indices_experts_sorted, num_local_tokens_per_expert, routing_weights, selected_experts_indices
...
class MoE(nn.Module):
    def __init__(self, model_args: TransformerModelArgs):
        ...
        self.use_triton_moe = model_args.use_triton_moe
        if self.use_triton_moe and fused_moe is None:
             print("Warning: use_triton_moe is True but fused_moe kernel could not be imported. Fallback to standard.")
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_triton_moe:
            flat_x = x.reshape(bs * slen, dim)
            # fused_moe expects: hidden_states, w1, w2, w3, topk_weights, topk_ids
            # weights are in self.experts.w1/w2/w3 [num_experts, dim, hidden_dim] etc
            # fused_moe signature: (hidden_states, w1, w2, w3, topk_weights, topk_ids)
            
            # Ensure topk_weights sums to 1? TokenChoiceTopKRouter uses softmax/sigmoid but doesn't normalize topk.
            # vLLM `fused_moe` typically expects weights to be pre-normalized if that's the logic.
            # Llama 4 logic: "The output of the experts is ... weighted by the router scores."
            # The standard logic uses `top_scores` which are gathered probabilities.
            # If `token_choice` routing assumes normalization, we should do it.
            # But `TokenChoiceTopKRouter` does NOT normalize `top_scores` after gather in the original code.
            # It just uses them. XPU/Triton kernel will multiply by these weights.
            ...
            if self.shared_expert is not None:
                shared_out = self.shared_expert(x.reshape(1, bs * slen, dim)).reshape(
                    bs * slen, dim
                )
                out = out + shared_out
